 ...
 */
//
const objectAssign = Object.assign; //data,events,i18n需要单独处理，只处理一层数据，所以建议尽量使用一层数据
...
function handlePageTo() {
  ...
  obj['reLaunchTo'] = obj['reLaunch']; //整合为pageTo方法
...
assign(wxapp, handlePageTo(), {
  ...
  go(page, query, type, cb) {
    ...
    const path = wxapp.PageNamePathKV[page] || page; // 进入网页
  ...
  */
  $set(data, callback) {
    ...
    if (now - wxapp.__SET_DATA_FREQUENCY_TIMESTAMP < wxapp.SET_DATA_FREQUENCY) {
      //
      wxapp.__SET_DATA_CACHE = assign(wxapp.__SET_DATA_CACHE || {}, data); //定时任务
  ...
  */
  bindEvent(e) {
    ...
    function _next() {
      ...
      const id = currentTarget.dataset.eventid || currentTarget.dataset.id || currentTarget.id; //console.log('id:',id);
  ...
  bindTelEvent(e) {
    ...
    const notAllowed = this.dataset('notAllowed', e); //拨打权限处理
      ...
      wx.authorize({
        ...
        fail: () => {
          ...
          wxapp.showModal({
            ...
            content: this.i18nHandle('authorize.content', {
              appName,
              scopeName
            }),
            showCancel: true,
...
let modalObj = null;
let isShowLoading = false; //消息提示相关
