...
const methods = {
  ...
  TE(e) {
    ...
    //
    let offset =
      this.data.TOUCH_POSITION_OFFSET ||
      this.TOUCH_POSITION_OFFSET ||
      TOUCH_POSITION_OFFSET;
    let timestamp =
      this.data.TOUCH_TIMESTAMP_OFFSET ||
      this.TOUCH_TIMESTAMP_OFFSET ||
      TOUCH_TIMESTAMP_OFFSET;
    //满足时间偏移
    if (delta.moveTimeStamp >= timestamp) {
      const absX = Math.abs(delta.moveX);
      const absY = Math.abs(delta.moveY);
